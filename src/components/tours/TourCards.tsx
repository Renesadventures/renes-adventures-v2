...
function hasHolChanRangerFeeNotice(tourSlug: string) {
  return tourSlug === 'custom-charter';
}
          ...
          {tours.map((tour) => (
            ...
            >
              ...
              <div className="p-7">
                ...
                {hasHolChanRangerFeeNotice(tour.slug) && (
                  <div className="mb-5 rounded-2xl border-2 border-[#D4AF37]/55 bg-black/25 p-4">
                    <div className="text-xs uppercase tracking-[0.35em] text-[#D4AF37] font-bold">Hol Chan Note</div>
                    <div className="mt-2 text-sm text-white font-bold leading-relaxed">
                      $15 Ranger Fee (paid directly to HCMR ranger at time of boat entry)
                    </div>
                  </div>
                )}
                ...
                <div className="flex justify-between items-start mb-5 pb-5 border-b border-white/10">
                  ...
                  <div className="text-right text-sm text-white/70">
                    <div className="font-medium text-white/80">Up to {tour.includedGuests} guests</div>
                    <div className="text-white/60">+ {formatMoney(tour.additionalGuestPrice)} / guest</div>
                  </div>
                </div>
