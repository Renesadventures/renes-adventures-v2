...
function pickByConditions(args: {
  ...
  if (args.windMph < 5 && args.precipitation < 20) {
    return {
      id: 'blue-hole',
      tourName: 'Blue Hole Adventure',
      headline: "CAPTAIN'S CHOICE",
      recommendation: 'Blue Hole — Pristine Visibility',
      reasoning: `Dead-calm winds (${args.windMph} mph) and gentle seas make the run comfortable and the water crystal clear. This is a rare perfect day for the Blue Hole.`,
      bookingHref: '/tours/blue-hole',
      defaultMediaId: 'img-blue-hole-iconic',
    };
  }

  // Protected north reef → Custom Charter snorkeling
  if (
    (args.windDir === 'ENE' || args.windDir === 'E' || args.windDir === 'NE') &&
    args.windMph >= 5 &&
    args.windMph <= 15 &&
    args.waveMaxFt < 3 &&
    args.precipitation < 30
  ) {
    return {
      id: 'custom-snorkel',
      tourName: "Rene's Custom Adventure",
      headline: "CAPTAIN'S CHOICE",
      recommendation: 'North Reef is Glass — Perfect Snorkeling',
      reasoning: `Winds from the ${args.windDir} at ${args.windMph} mph keep the north side protected. Crystal-clear visibility for Hol Chan, sharks, rays, and seahorses.`,
  ...
  if (args.precipitation >= 40 && args.windMph <= 15) {
    return {
      id: 'deep-sea-rain',
      tourName: 'Deep Sea Fishing',
      headline: "CAPTAIN'S CHOICE",
      recommendation: 'Rain Means the Fish Are Biting',
      reasoning: `${args.precipitation}% chance of rain, but experienced captains know: overcast skies and choppy water drive pelagics to feed. Mahi-mahi and barracuda will be active.`,
      bookingHref: '/tours/deep-sea-fishing',
      defaultMediaId: 'vid-deep-sea-5',
    };
  }

  // Choppy / windy → Deep Sea (big game thrives)
  if (args.windMph > 15 || args.waveMaxFt >= 3) {
    return {
      ...
      reasoning: `Winds over ${args.windMph} mph push bait into channels. Pelagics stay active and the bite turns on fast. Captain Rene knows the lee-side spots.`,
      bookingHref: '/tours/deep-sea-fishing',
      defaultMediaId: 'vid-deep-sea-5',
    };
  }

  // Calm evening → Sunset Cruise
  if (args.windMph <= 10 && args.precipitation < 20) {
    return {
      id: 'sunset',
      tourName: 'Sunset Cruise',
      headline: "CAPTAIN'S CHOICE",
      recommendation: 'Golden Hour on Glass Water',
      reasoning: `Light winds at ${args.windMph} mph and clear skies. Tonight's sunset will be unforgettable — champagne, calm waters, and the Belize sky doing its thing.`,
...
export default function ConditionsWidget() {
  ...
  const pick = useMemo(() => {
    if (!selectedDay) return pickByConditions({ windMph: 12, windDir: 'ENE', waveMaxFt: 2.5, precipitation: 15 });
